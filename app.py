...
with st.sidebar:
    ...
    # Nav radio kaldırıldı — tab sistemi kullanılıyor
    pass  # sidebar nav placeholder

    # ── Method özeti ─────────────────────────────────────────────────────────
    st.markdown(
        f"""<div style="background:rgba(0,17,42,0.6);border:1px solid rgba(255,191,0,0.15);
                        border-radius:8px;padding:10px 14px;margin:8px 0 12px 0;font-size:0.72rem;
                        color:rgba(232,224,208,0.6);line-height:1.9;">
          <div style="color:rgba(255,191,0,0.7);font-weight:600;margin-bottom:4px;font-size:0.68rem;
                      letter-spacing:0.09em;text-transform:uppercase;">Method</div>
    ...
    n_prof = len(st.session_state.profiles)
    if n_prof > 0:
        _pnames = list(st.session_state.profiles.keys())
        _pdisp  = ", ".join(_pnames[:3]) + (f" +{n_prof-3}" if n_prof > 3 else "")
        st.markdown(
            f'<div style="font-size:0.69rem;color:rgba(232,224,208,0.42);padding:0 2px 10px;">'
            f'📊 {n_prof} profile · {_pdisp}</div>',
            unsafe_allow_html=True
        )
    st.markdown('<hr style="border:1px solid rgba(255,191,0,0.1);margin:4px 0 10px;">', unsafe_allow_html=True)
    if st.button("🗑️ New Project", use_container_width=True, help="Clear all data and start fresh."):
    ...
    if st.session_state.get("_confirm_new"):
        ...
        with _c1:
            if st.button("✅ Yes", use_container_width=True):
                _clear_all(); st.session_state["_confirm_new"] = False; st.rerun()
        with _c2:
            if st.button("❌ No", use_container_width=True):
                st.session_state["_confirm_new"] = False; st.rerun()
    st.markdown(
...
# ── TAB 3: SETTINGS ───────────────────────────────────────────────────────────
with _main_tabs[2]:
    _settings_tabs = st.tabs(["⚙️ Method Settings", "🔬 Analytical Settings", "📚 References"])
