...
if st.session_state.page == "landing":
    ...
    .stButton > button {
        background: #FFBF00 !important; color: #001a3d !important;
        border: none !important; font-family: Inter, sans-serif !important;
        font-size: 1.1rem !important; font-weight: 700 !important;
        padding: 14px 40px !important; border-radius: 6px !important;
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    .lp { font-family: Inter, sans-serif; background: #000d1a; min-height: 100vh; color: #e8e0d0; }
    .lp::before {
        content: ""; position: fixed; inset: 0;
        background-image: linear-gradient(rgba(255,191,0,0.03) 1px, transparent 1px),
            linear-gradient(90deg, rgba(255,191,0,0.03) 1px, transparent 1px);
        background-size: 60px 60px; pointer-events: none; z-index: 0;
    }
    .lp-glow1 { position:fixed; top:-200px; left:-200px; width:700px; height:700px;
        background:radial-gradient(circle,rgba(0,33,71,0.7) 0%,transparent 70%);
        pointer-events:none; z-index:0; }
    .lp-glow2 { position:fixed; bottom:-200px; right:-100px; width:600px; height:600px;
        background:radial-gradient(circle,rgba(255,191,0,0.07) 0%,transparent 70%);
        pointer-events:none; z-index:0; }
    .lp-nav { position:relative; z-index:10; display:flex; align-items:center;
    ...
    .lp-logo { font-size:1.4rem; font-weight:900; color:#FFBF00; letter-spacing:-0.02em; }
    .lp-badge { background:rgba(255,191,0,0.12); border:1px solid rgba(255,191,0,0.3);
        color:#FFBF00; font-size:0.6rem; font-weight:700; letter-spacing:0.12em;
        text-transform:uppercase; padding:3px 8px; border-radius:20px; }
    .lp-nav-links { display:flex; gap:32px; }
    .lp-nav-links a { color:rgba(232,224,208,0.6); text-decoration:none;
        font-size:0.85rem; font-weight:500; }
    .lp-hero { position:relative; z-index:1; display:grid;
        grid-template-columns:1fr 1fr; align-items:center;
        gap:60px; padding:80px 60px 60px; min-height:86vh; }
    .lp-left { display:flex; flex-direction:column; gap:24px; }
    .lp-eyebrow { display:inline-flex; align-items:center; gap:8px;
        background:rgba(255,191,0,0.08); border:1px solid rgba(255,191,0,0.25);
        color:#FFBF00; font-size:0.72rem; font-weight:700; letter-spacing:0.14em;
        text-transform:uppercase; padding:6px 14px; border-radius:20px; width:fit-content; }
    .lp-dot { width:6px; height:6px; background:#FFBF00; border-radius:50%;
        animation:pulse 2s infinite; flex-shrink:0; }
    @keyframes pulse {{ 0%,100%{{opacity:1;transform:scale(1)}} 50%{{opacity:.4;transform:scale(.8)}} }}
    .lp-h1 { font-size:4.2rem; font-weight:900; line-height:1.05;
        letter-spacing:-0.03em; color:#fff; }
    .lp-h1 span { color:#FFBF00; }
    .lp-sub { font-size:1.05rem; color:rgba(232,224,208,0.65);
        line-height:1.7; max-width:480px; }
    .lp-stats { display:flex; gap:32px; margin-top:16px; padding-top:24px;
        border-top:1px solid rgba(255,255,255,0.06); }
    .lp-stat-n { font-size:1.8rem; font-weight:900; color:#FFBF00; letter-spacing:-0.02em; }
    .lp-stat-l { font-size:0.7rem; color:rgba(232,224,208,0.5);
        text-transform:uppercase; letter-spacing:0.08em; font-weight:500; }
    .lp-right { display:flex; align-items:center; justify-content:center; position:relative; }
    .lp-pw { position:relative; width:420px; }
    .lp-pglow { position:absolute; inset:-40px;
        background:radial-gradient(ellipse at center,rgba(255,191,0,0.18) 0%,transparent 65%);
        border-radius:50%; filter:blur(20px); z-index:0; }
    .lp-pimg { position:relative; z-index:1; width:100%; border-radius:16px;
        border:1.5px solid rgba(255,191,0,0.35);
        box-shadow:0 0 0 1px rgba(255,191,0,0.1),0 40px 80px rgba(0,0,0,0.6),0 0 60px rgba(255,191,0,0.08); }
    .lp-fb { position:absolute; z-index:2; background:rgba(0,13,26,0.92);
        border:1px solid rgba(255,191,0,0.3); border-radius:8px;
        padding:10px 14px; backdrop-filter:blur(10px); }
    .lp-fb-tl { top:-16px; left:-20px; }
    .lp-fb-br { bottom:20px; right:-24px; }
    .lp-fb-l { font-size:0.62rem; color:rgba(255,191,0,0.7);
        text-transform:uppercase; letter-spacing:0.1em; font-weight:600; }
    .lp-fb-v { font-size:1rem; color:#fff; font-weight:700; margin-top:1px; }
    .lp-sec { position:relative; z-index:1; padding:80px 60px;
        border-top:1px solid rgba(255,255,255,0.05); }
    .lp-sec-lbl { font-size:0.7rem; color:#FFBF00; text-transform:uppercase;
        letter-spacing:0.15em; font-weight:700; margin-bottom:12px; }
    .lp-sec-h { font-size:2.6rem; font-weight:800; color:#fff;
        letter-spacing:-0.02em; margin-bottom:8px; line-height:1.1; }
    .lp-sec-sub { font-size:1rem; color:rgba(232,224,208,0.55);
        margin-bottom:48px; max-width:520px; }
    .lp-cards { display:grid; grid-template-columns:repeat(4,1fr); gap:20px; }
    .lp-card { background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.07);
        border-radius:12px; padding:24px; transition:all 0.25s; }
    .lp-card:hover { background:rgba(255,191,0,0.05); border-color:rgba(255,191,0,0.25);
        transform:translateY(-3px); box-shadow:0 20px 40px rgba(0,0,0,0.3); }
    .lp-ci { width:44px; height:44px; background:rgba(255,191,0,0.1);
        border:1px solid rgba(255,191,0,0.2); border-radius:10px;
        display:flex; align-items:center; justify-content:center;
        margin-bottom:16px; font-size:1.3rem; }
    .lp-ct { font-size:0.95rem; font-weight:700; color:#fff; margin-bottom:8px; }
    .lp-cd { font-size:0.8rem; color:rgba(232,224,208,0.5); line-height:1.6; }
    .lp-pgrid { display:grid; grid-template-columns:repeat(3,1fr); gap:24px; margin-top:48px; }
    .lp-plan { background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08);
        border-radius:14px; padding:32px; position:relative; }
    .lp-pf { background:rgba(0,33,71,0.6); border:1.5px solid rgba(255,191,0,0.4);
        box-shadow:0 0 40px rgba(255,191,0,0.06); }
    .lp-ptag { position:absolute; top:-12px; left:50%; transform:translateX(-50%);
        background:#FFBF00; color:#001a3d; font-size:0.62rem; font-weight:800;
        text-transform:uppercase; letter-spacing:0.1em;
        padding:4px 14px; border-radius:20px; white-space:nowrap; }
    .lp-pname { font-size:0.8rem; font-weight:700; text-transform:uppercase;
        letter-spacing:0.12em; color:rgba(232,224,208,0.6); margin-bottom:10px; }
    .lp-pprice { font-size:2.8rem; font-weight:900; color:#fff;
        letter-spacing:-0.03em; line-height:1; }
    .lp-pper { font-size:0.8rem; color:rgba(232,224,208,0.4);
        margin-bottom:24px; margin-top:4px; }
    .lp-pf-list { list-style:none; margin-bottom:28px; }
    .lp-pf-list li { font-size:0.82rem; color:rgba(232,224,208,0.65);
        padding:7px 0; border-bottom:1px solid rgba(255,255,255,0.05);
        display:flex; align-items:center; gap:8px; }
    .lp-pf-list li::before { content:""; width:5px; height:5px;
        background:#FFBF00; border-radius:50%; flex-shrink:0; }
    .lp-pbtn { width:100%; padding:12px; border-radius:6px; font-size:0.88rem;
        font-weight:700; cursor:pointer; border:none; text-align:center;
        transition:all 0.2s; font-family:Inter,sans-serif; }
    .lp-pbtn-on { background:#FFBF00; color:#001a3d; }
    .lp-pbtn-off { background:rgba(255,255,255,0.05); color:rgba(255,255,255,0.3);
        cursor:not-allowed; border:1px solid rgba(255,255,255,0.08); }
    .lp-sbadge { display:inline-flex; align-items:center; gap:5px; font-size:0.62rem;
        font-weight:700; text-transform:uppercase; letter-spacing:0.1em;
        padding:3px 8px; border-radius:20px; margin-bottom:6px; }
    .lp-sa { background:rgba(52,211,153,0.12); color:#34d399; border:1px solid rgba(52,211,153,0.25); }
    .lp-sb { background:rgba(99,102,241,0.12); color:#818cf8; border:1px solid rgba(99,102,241,0.25); }
    .lp-ss { background:rgba(148,163,184,0.08); color:rgba(148,163,184,0.6); border:1px solid rgba(148,163,184,0.15); }
    .lp-footer { position:relative; z-index:1; padding:32px 60px;
        border-top:1px solid rgba(255,255,255,0.05);
        display:flex; align-items:center; justify-content:space-between; }
    .lp-fc { font-size:0.75rem; color:rgba(232,224,208,0.3); }
    .lp-fa { font-size:0.75rem; color:rgba(232,224,208,0.4); text-align:right; }
    </style>

    <div class="lp">
      <div class="lp-glow1"></div><div class="lp-glow2"></div>

      <nav class="lp-nav">
        <div class="lp-brand">
          <span class="lp-logo">DissolvA&trade;</span>
          <span class="lp-badge">Powered by AI</span>
        </div>
        <div class="lp-nav-links">
          <a href="#">Features</a><a href="#">Scientific Core</a>
          <a href="#">Pricing</a>
          <a href="mailto:msinankaynak@gmail.com">Contact</a>
        </div>
      </nav>

      <section class="lp-hero">
        <div class="lp-left">
          <div class="lp-eyebrow"><span class="lp-dot"></span>Regulatory-Grade Dissolution Analysis</div>
          <h1 class="lp-h1">Unlock<br><span>Predictive</span><br>Intelligence.</h1>
          <p class="lp-sub">DissolvA&trade; is the enterprise dissolution modeling suite
            built for pharmaceutical scientists. 48+ kinetic models, Bootstrap f2,
            IVIVC analysis, and AI-driven insights &mdash; all in one platform.</p>
          <div class="lp-stats">
            <div><div class="lp-stat-n">48+</div><div class="lp-stat-l">Kinetic Models</div></div>
            <div><div class="lp-stat-n">6</div><div class="lp-stat-l">Categories</div></div>
            <div><div class="lp-stat-n">FDA</div><div class="lp-stat-l">f1 &amp; f2</div></div>
            <div><div class="lp-stat-n">IVIVC</div><div class="lp-stat-l">Wagner-Nelson</div></div>
          </div>
        </div>
        <div class="lp-right">
          <div class="lp-pw">
            <div class="lp-pglow"></div>
            <img class="lp-pimg" src="data:image/jpeg;base64,{LOGO_B64}" alt="DissolvA"/>
            <div class="lp-fb lp-fb-tl">
              <div class="lp-fb-l">Kinetic Models</div>
              <div class="lp-fb-v">48+ Active</div>
            </div>
            <div class="lp-fb lp-fb-br">
              <div class="lp-fb-l">Regulatory</div>
              <div class="lp-fb-v">FDA / USP</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="lp-sec" id="features">
        <div class="lp-sec-lbl">Core Capabilities</div>
        <div class="lp-sec-h">Scientific Core</div>
        <div class="lp-sec-sub">Every module built on peer-reviewed methodology and validated against published literature.</div>
        <div class="lp-cards">
          <div class="lp-card">
            <div class="lp-ci"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#FFBF00" stroke-width="2"><path d="M3 3v18h18"/><path d="m19 9-5 5-4-4-3 3"/></svg></div>
            <div class="lp-ct">Kinetic Model Fitting</div>
            <div class="lp-cd">48+ models across 6 categories. Zero Order to Fractal Weibull, Peppas-Sahlin, Baker-Lonsdale. Ranked by R&sup2;adj, AIC, MSC.</div>
          </div>
          <div class="lp-card">
            <div class="lp-ci"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#FFBF00" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M4.22 4.22l2.12 2.12M17.66 17.66l2.12 2.12M2 12h3M19 12h3M4.22 19.78l2.12-2.12M17.66 6.34l2.12-2.12"/></svg></div>
            <div class="lp-ct">Bootstrap f2 Analysis</div>
            <div class="lp-cd">FDA-compliant f2 &ge;50 and f1 &le;15 with full point-by-point comparison and LaTeX equations.</div>
          </div>
          <div class="lp-card">
            <div class="lp-ci"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#FFBF00" stroke-width="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg></div>
            <div class="lp-ct">IVIVC Analysis</div>
            <div class="lp-cd">Wagner-Nelson method for in vitro to in vivo correlation. Fraction absorbed estimation with full PK input.</div>
          </div>
          <div class="lp-card">
            <div class="lp-ci"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#FFBF00" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 9h6M9 12h6M9 15h4"/></svg></div>
            <div class="lp-ct">Regulatory Reporting</div>
            <div class="lp-cd">Professional Excel reports with dissolution charts, method documentation and similarity analysis in one click.</div>
          </div>
        </div>
      </section>

      <section class="lp-sec" id="pricing">
        <div class="lp-sec-lbl">Pricing</div>
        <div class="lp-sec-h">Plans &amp; Access</div>
        <div class="lp-sec-sub">Start with the Academic Plan today. Professional and Enterprise tiers are in active development.</div>
        <div class="lp-pgrid">
          <div class="lp-plan lp-pf">
            <div class="lp-ptag">Most Popular</div>
            <div class="lp-sbadge lp-sa"><span style="width:6px;height:6px;background:#34d399;border-radius:50%;display:inline-block;"></span>Active</div>
            <div class="lp-pname">Academic Plan</div>
            <div class="lp-pprice">Free</div>
            <div class="lp-pper">Open access &bull; Streamlit Cloud</div>
            <ul class="lp-pf-list">
              <li>48+ Kinetic Models</li><li>f1 &amp; f2 Similarity Analysis</li>
              <li>IVIVC Wagner-Nelson</li><li>Statistical Profiling (MDT, DE)</li>
              <li>Excel Report Export</li><li>Method &amp; Parameter Settings</li>
              <li>6 or 12 vessel dissolution</li>
            </ul>
          </div>
          <div class="lp-plan">
            <div class="lp-sbadge lp-sb">Beta</div>
            <div class="lp-pname">Professional Plan</div>
            <div class="lp-pprice">$49</div>
            <div class="lp-pper">per month &bull; coming soon</div>
            <ul class="lp-pf-list">
              <li>Everything in Academic</li><li>Batch processing</li>
              <li>Advanced Bootstrap f2</li><li>Custom model library</li>
              <li>Priority support</li><li>PDF regulatory reports</li>
            </ul>
            <button class="lp-pbtn lp-pbtn-off" disabled>Under Development</button>
          </div>
          <div class="lp-plan">
            <div class="lp-sbadge lp-ss">Coming Soon</div>
            <div class="lp-pname">Enterprise Plan</div>
            <div class="lp-pprice">Custom</div>
            <div class="lp-pper">annual contract &bull; on-premise</div>
            <ul class="lp-pf-list">
              <li>Everything in Professional</li><li>On-premise deployment</li>
              <li>ELN / LIMS integration</li><li>21 CFR Part 11 compliance</li>
              <li>Dedicated support team</li><li>Custom AI model training</li>
            </ul>
            <button class="lp-pbtn lp-pbtn-off" disabled>Contact Us</button>
          </div>
        </div>
      </section>

      <footer class="lp-footer">
        <div class="lp-fc">&copy; 2025 DissolvA&trade; &mdash; Predictive Dissolution Suite. All rights reserved.</div>
        <div class="lp-fa">
          Developed by <strong style="color:#FFBF00;">M. Sinan KAYNAK, PhD</strong><br>
          Anadolu University, Faculty of Pharmacy &nbsp;&bull;&nbsp;
          <a href="mailto:msinankaynak@gmail.com" style="color:rgba(232,224,208,0.5);text-decoration:none;">msinankaynak@gmail.com</a>
        </div>
      </footer>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_btn:
        if st.button("Launch Analysis  ->", type="primary",
                     key="landing_launch", use_container_width=True):
            st.session_state.page = "app"
            st.rerun()
    st.stop()

# Restore sidebar for app
st.markdown("""
<style>
