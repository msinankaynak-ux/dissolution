...
def render():
    ...
    if _as_km.get("fetch_done") and _as_km.get("name"):
        ...
        _sel_fda = (
            _fda_km[_sel_km]
            if (_sel_km is not None and _sel_km < len(_fda_km))
            else None
        )
    ...
    _has_sd = (
        _sd_arr is not None and _sd_arr.size == len(r_arr) and bool(np.any(_sd_arr > 0))
    )
    ...
    if st.session_state.fit_results:
        ...
        res_fail = {
            k: v for k, v in st.session_state.fit_results.items() if not v["success"]
        }
        ...
        rows = [
            {
                ...
                "R2": _rn(v.get("r2"), 4),
                "R2adj": _rn(v.get("r2adj"), 4),
                "RMSE": _rn(v.get("rmse"), 3),
                "AIC": _rn(v.get("aic"), 2),
                "AICc": _rn(v.get("aicc"), 2),
                "BIC": _rn(v.get("bic"), 2),
                "MSC": _rn(v.get("msc"), 3),
                "Params": v.get("n_params", 0),
        ...
        if rows:
            ...
            df_r = (
                pd.DataFrame(rows)
                .sort_values(_sort_col, ascending=_rank_asc)
                .reset_index(drop=True)
            )
            df_r.index += 1
            _grad_cmap = (
                "YlGn_r" if _rank_asc else "YlGn"
            )  # küçük=iyi ise ters renk skalası
            st.dataframe(
                df_r.style.background_gradient(subset=[_sort_col], cmap=_grad_cmap),
                ...
                _tx_rows.append(
                    {
                        ...
                        f"T25 ({time_unit})": (
                            _rn(_tx.get("25"), 2)
                            if _tx.get("25") is not None
                            else "n/a"
                        ),
                        f"T50 ({time_unit})": (
                            _rn(_tx.get("50"), 2)
                            if _tx.get("50") is not None
                            else "n/a"
                        ),
                        f"T80 ({time_unit})": (
                            _rn(_tx.get("80"), 2)
                            if _tx.get("80") is not None
                            else "n/a"
                        ),
                        f"T90 ({time_unit})": (
                            _rn(_tx.get("90"), 2)
                            if _tx.get("90") is not None
                            else "n/a"
                        ),
                    }
                )
        ...
        for i, (mn, v) in enumerate(res_ok.items()):
            try:
                ...
                _lbl = (
                    f"{mn} (R2adj={_r2a:.3f})"
                    if isinstance(_r2a, (int, float)) and _r2a == _r2a
                    else mn
                )
                ax.plot(
                    xs,
        ...
        if _curve_skip:
            st.caption(
                f"Curves not shown for: {', '.join(_curve_skip)} (non-finite parameters)."
            )
        ...
        for mn, v in res_ok.items():
            with st.expander(
                f"{mn}  |  R2adj={_fmt(v.get('r2adj'),4)}  |  "
                f"RMSE={_fmt(v.get('rmse'),3)}  |  AICc={_fmt(v.get('aicc'),2)}"
            ):
                st.markdown(
                    f"<div class='eq-box'>{v.get('equation','')}</div>",
                    unsafe_allow_html=True,
                )
                ...
                if v.get("bounds_enforced") is False:
                    st.caption(
                        "⚠️ Unconstrained fit — physical bounds could not be applied; "
                        "interpret parameters with caution."
                    )
                _nanf = v.get("nan_fraction") or 0
                if _nanf > 0.10:
                    st.caption(
                        f"⚠️ {_nanf:.0%} of model predictions are undefined — "
        ...
        _diag_models = [
            m
            for m, v in res_ok.items()
            if (v.get("diagnostics") or {}).get("residuals")
        ]
        if _diag_models:
            with st.expander("Residual Diagnostics"):
                _best = st.session_state.get("fit_best")
                _default_idx = (
                    _diag_models.index(_best) if (_best in _diag_models) else 0
                )
                _dm = st.selectbox(
                    "Model",
                    _diag_models,
