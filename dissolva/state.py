...
def _rename_profile(old_name: str, new_name: str):
    ...
    if (
        old_name == new_name
        or new_name.strip() == ""
        or old_name not in st.session_state.profiles
    ):
        return False
    ...
    if old_name in st.session_state.fit_results:
        st.session_state.fit_results[new_name] = st.session_state.fit_results.pop(
            old_name
        )
...
def _clear_all():
    ...
    # Uyum modu — aktif kayit baglamini temizle
    for _k in ("_active_record_id", "_active_record_name", "_show_saveas"):
